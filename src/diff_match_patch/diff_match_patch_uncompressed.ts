 ...
 * @constructor
 */
class diff_match_patch_defaults {
  // Defaults.
...
export const diff_main = function (
  ...
  if (typeof opt_deadline == "undefined") {
    if (diff_match_patch_defaults.Diff_Timeout <= 0) {
      ...
      opt_deadline =
        new Date().getTime() + diff_match_patch_defaults.Diff_Timeout * 1000;
    }
  }
  ...
  var diffs = diff_compute_(text1, text2, checklines, deadline);
...
export const diff_compute_ = function (
  ...
  if (checklines && text1.length > 100 && text2.length > 100) {
    return diff_lineMode_(text1, text2, deadline);
  }

  return diff_bisect_(text1, text2, deadline);
};
...
export const diff_lineMode_ = function (
  ...
  var a = diff_linesToChars_(text1, text2);
  text1 = a.chars1;
  text2 = a.chars2;
    ...
    for (var k1 = -d + k1start; k1 <= d - k1end; k1 += 2) {
      ...
      if (x1 > text1_length) {
        ...
        if (k2_offset >= 0 && k2_offset < v_length && v2[k2_offset] != -1) {
          ...
          if (x1 >= x2) {
            // Overlap detected.
            return diff_bisectSplit_(text1, text2, x1, y1, deadline);
          }
        }
      }
    }
...
export const diff_halfMatch_ = function (
  ...
  var hm1: any = diff_halfMatchI_(
    longtext,
    shorttext,
    Math.ceil(longtext.length / 4)
  );
  // Check again based on the third quarter.
  var hm2: any = diff_halfMatchI_(
    longtext,
    shorttext,
...
export const diff_cleanupEfficiency = function (diffs: Diff[]) {
  ...
  var pre_ins: any = false;
  // Is there a deletion operation before the last equality.
  var pre_del: any = false;
  // Is there an insertion operation after the last equality.
  var post_ins: any = false;
  // Is there a deletion operation after the last equality.
  var post_del: any = false;
  while (pointer < diffs.length) {
    if (diffs[pointer].op == DIFF_EQUAL) {
      // Equality found.
      if (
        diffs[pointer].text.length < diff_match_patch_defaults.Diff_EditCost &&
        (post_ins || post_del)
      ) {
        // Candidate found.
        equalities[equalitiesLength++] = pointer;
      ...
      if (
        ...
        ((pre_ins && pre_del && post_ins && post_del) ||
          (lastEquality.length < diff_match_patch_defaults.Diff_EditCost / 2 &&
...
export const match_main = function (
  ...
  if (text == pattern) {
    ...
    // Do a fuzzy compare.
    return match_bitap_(text, pattern, loc);
  }
};
...
export const match_bitap_ = function (
  ...
  var s: any = match_alphabet_(pattern);
  ...
  function match_bitapScore_(e: number, x: number) {
    ...
    if (!diff_match_patch_defaults.Match_Distance) {
      // Dodge divide by zero error.
      return proximity ? 1.0 : accuracy;
    }
    return accuracy + proximity / diff_match_patch_defaults.Match_Distance;
  }

  // Highest score beyond which we give up.
  var score_threshold = diff_match_patch_defaults.Match_Threshold;
...
export const patch_make = function (a: any, opt_b: any, opt_c: any) {
  ...
  if (
    ...
    text1 = /** @type {string} */ a;
    diffs = diff_main(text1, /** @type {string} */ opt_b, true);
    if (diffs.length > 2) {
      diff_cleanupSemantic(diffs);
...
export const patch_apply = function (patches: patch_obj[], text: string) {
  ...
  for (var x = 0; x < patches.length; x++) {
    ...
    var start_loc;
    var end_loc = -1;
    if (text1.length > diff_match_patch_defaults.Match_MaxBits) {
      // patch_splitMax will only provide an oversized pattern in the case of
      // a monster delete.
      start_loc = match_main(
        text,
        text1.substring(0, diff_match_patch_defaults.Match_MaxBits),
        expected_loc
      );
      if (start_loc != -1) {
        end_loc = match_main(
      ...
      if (text1 == text2) {
        ...
        if (
          ...
          diff_levenshtein(diffs) / text1.length >
            diff_match_patch_defaults.Patch_DeleteThreshold
        ) {
          ...
          var index1 = 0;
          var index2: any;
          for (var y = 0; y < (patches[x].diffs as any).length; y++) {
            var mod = (patches[x].diffs as any)[y];
            if (mod[0] !== DIFF_EQUAL) {
            ...
            if (mod[0] === DIFF_INSERT) {
              ...
              text =
                text.substring(0, start_loc + index2) +
                text.substring(
                  start_loc + diff_xIndex(diffs, index1 + mod[1].length)
                );
            }
  ...
  for (var x = 0; x < patches.length; x++) {
    ...
    while (bigpatch.diffs.length !== 0) {
      ...
      while (
        ...
        patch.length1 < patch_size - diff_match_patch_defaults.Patch_Margin
      ) {
        ...
        if (diff_type === DIFF_INSERT) {
          ...
          diff_text = diff_text.substring(
            0,
            patch_size - patch.length1 - diff_match_patch_defaults.Patch_Margin
          );
          patch.length1 += diff_text.length;
          start1 += diff_text.length;
  ...
  while (textPointer < text.length) {
    ...
    while (textPointer < text.length) {
      ...
      if (sign == "-") {
        // Deletion.
        // @ts-ignore
        patch.diffs.push(new Diff(DIFF_DELETE, line));
      } else if (sign == "+") {
        // Insertion.
        // @ts-ignore
        patch.diffs.push(new Diff(DIFF_INSERT, line));
      } else if (sign == " ") {
        // Minor equality.
        // @ts-ignore
        patch.diffs.push(new Diff(DIFF_EQUAL, line));
      } else if (sign == "@") {
        // Start of next patch.
        break;
      } else if (sign === "") {
